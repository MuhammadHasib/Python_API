...
for m in measurements:
	...
	RUN_TYPE = m[2]
	RUN_NUMBER = m[3]
        list_IMON_UA.append( m[7] )
        list_VMON_VLT.append( m[5] )
	list_RATE_HZ.append( m[11] * 200.0 )
	list_ERR_RATE_HZ.append( m[12] * 200.0 )
	# print m # uncomment to debug
		...
		img = TImage.Create()
                img.FromPad(c)
                img.WriteImage('gQC4_' + str(graphCount) + '_' + old_PART_SERIAL_NUMBER.replace('/','-') + '.png')
                # updates the lists to only have the values of the last measurement
                list_IMON_UA = [ m[1] ]
                list_VMON_VLT = [ m[2] ]
		list_RATE_HZ = [ m[3] * 200.0 ]
		list_ERR_RATE_HZ = [ m[4] * 200.0]
		c.Close()		
